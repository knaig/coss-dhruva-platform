...
def log_to_storage(
    ...
    """Log input output data pairs to MongoDB"""
    try:
        log_store.logs.insert_one({
            "client_ip": client_ip,
            "error_msg": error_msg,
        ...
        logging.info(f"Successfully logged data for service {service_id}")
    ...
    try:
        log_store.logs.insert_one(log_document)
        logging.info(f"Successfully logged data for service {service_id}")
    except Exception as e:
        logging.error(f"Failed to log data: {str(e)}")
