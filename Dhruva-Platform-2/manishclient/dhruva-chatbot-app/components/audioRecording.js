/**
 * Simplified Audio Recording Utility for Dhruva Chatbot (JavaScript)
 * 
 * This utility provides a robust audio recording solution with automatic
 * format conversion and error handling for the chatbot component.
 */

// Audio recording configuration defaults
...
export class AudioRecorder {
   ...
   */
  getOptimalMediaRecorderOptions() {
    const options = {};
   ...
   */
  setupMediaRecorderEvents(sessionId) {
  ...
  /**
   * Convert audio blob to 16kHz WAV format for optimal ASR compatibility
   */
  async convertToWav(audioBlob) {
    return await this.convertTo16kHzMono(audioBlob);
  }
   ...
   */
  createError(code, message, retryable = true) {
    const userMessages = {
 ...
 */
export async function processUploadedAudioFile(file, preferredFormat = 'wav') {
  ...
  try {
    ...
    const convertedBlob = await recorder.convertTo16kHzMono(file);
 ...
 */
export async function convertTo16kHzMono(blob) {
  const recorder = new AudioRecorder();
  return await recorder.convertTo16kHzMono(blob);
}
